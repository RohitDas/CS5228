...
def read_test_titles():
    ...
    with open("test_v2.csv", "r") as fp:
        ...
        for line in fp:
            ...
            print(csv_row)
            titles.update({
                csv_row[0]: csv_row[1].strip()  # csv_row[0] = ID, csv_row[1] = Title
            })
...
# print(test_keywords[0])
# test_df = pd.read_csv ("test_v2.csv", skiprows=[0], header=None)
# test_titles = preprocess(test_df[test_df.columns[1]])

# Test Train Split
# X_train, X_test, y_train, y_test = model_selection.train_test_split(keywords, labels, test_size=0.20)
# print(len(X_test))
# print(len(y_test))
...
# Save submission csv
np.save('seq_matcher_submission_n_5_old.npy', np.array(y_pred))
